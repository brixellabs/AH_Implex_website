...
export default function SocialSection() {
  ...
  const socialCards = [
    {
      ...
      handle: '@ah-impex • Textile Manufacturing & Global Sourcing',
      description: 'Connect with our Director of Export, supply chain merchandisers, and procurement team. Get official corporate updates, mill compliance audits, and container shipping bulletins.',
      bullets: [
        'Monthly Export Dispatch Reports',
        'OEKO-TEX & BSCI Certification Updates',
    ...
    },
    {
      ...
      bullets: [
        ...
        'Track Suit & Chino Fitting Reels',
        'Fast DM Response for Sample Kits'
      ],
    ...
    },
    {
      ...
      handle: '@ahimpextextiles • Customer Reviews & Catalog Albums',
      description: 'Review buyer feedback, view photo albums of our finished consignments for American & European retailers, and message our trade representatives directly via Messenger.',
      bullets: [
        'Live Messenger Chat Support',
        'Verified Client Feedback & Ratings',
        'New Season Color Palettes & Catalogs'
      ],
    ...
    {
      id: 'whatsapp',
      name: 'Direct WhatsApp Trade Desk',
      platform: 'Instant Trade Desk',
      handle: `${whatsappFormatted} • Fast Merchandising Response`,
      ...
      bullets: [
        'Instant Container Volume Costing',
        'Fabric Lab Dip & Swatch Tracking',
        'Real-Time Loom Scheduling Updates'
      ],
  ...
  return (
    <section id="social-hub" className="py-20 bg-gradient-to-b from-white to-slate-100 relative overflow-hidden border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Section Header (Matching Reference Design) */}
        <div className="text-center max-w-3xl mx-auto mb-14">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-brand-50 border border-brand-200 text-brand-700 text-xs font-bold tracking-widest uppercase mb-3 font-mono">
            <FontAwesomeIcon icon={faShareNodes} className="text-xs text-brand-600" />
            <span>Real-Time Factory &amp; Trade Updates</span>
          </div>

          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 tracking-tight font-serif leading-tight">
        ...
        {/* 4 Cards Grid with Exact Reference Design Buttons */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {socialCards.map((card) => (
            <div
              key={card.id}
              className={`rounded-2xl p-6 bg-white border border-slate-200 ${card.borderHover} transition-all duration-300 flex flex-col justify-between shadow-sm hover:shadow-2xl group`}
            >
              <div>
                {/* Card Top: Icon & Platform Badge */}
                <div className="flex items-center justify-between mb-5">
                  <div className={`w-12 h-12 rounded-xl ${card.iconContainer} flex items-center justify-center text-xl shadow-sm group-hover:scale-105 transition-transform`}>
                    <FontAwesomeIcon icon={card.icon} />
                  </div>
                  <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${card.badgeClass} font-mono`}>
                    {card.platform}
                  </span>
                </div>

                {/* Title & Handle */}
                <h3 className="text-lg font-bold text-slate-900 mb-1 font-serif group-hover:text-brand-700 transition-colors">
                  {card.name}
                </h3>
                <p className="text-xs text-brand-700 font-semibold mb-3 font-mono line-clamp-1">
                  {card.handle}
                </p>
                ...
                <div className="space-y-1.5 text-[11px] text-slate-600 mb-5 bg-slate-50 p-3 rounded-xl border border-slate-200">
              ...
              {/* Exact Colored Action Button */}
              <a
                href={card.url}
                target="_blank"
                rel="noopener noreferrer"
                className={`w-full ${card.btnClass} text-xs font-bold uppercase tracking-wider py-3 rounded-xl transition-all duration-300 text-center shadow-md flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-[0.98] font-mono`}
              >
