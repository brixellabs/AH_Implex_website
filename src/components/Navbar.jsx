...
export default function Navbar({ onOpenQuoteModal, onOpenAdmin }) {
  ...
  return (
    <>
      {/* Main Sticky Navigation Container */}
      <header
        role="banner"
        className={`sticky top-0 z-40 w-full transition-all duration-300 ${
          isScrolled
            ? 'glass-nav border-b border-brand-700/80 shadow-2xl py-2'
            : 'glass-nav border-b border-brand-700/50 py-2 sm:py-2.5 shadow-lg'
        }`}
      >
        <div className="w-full px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-2 lg:gap-3 xl:gap-6 w-full">
            ...
            <div className="lg:hidden flex items-center gap-1.5 sm:gap-2">
              <button
                onClick={() => onOpenQuoteModal()}
                className="px-2.5 sm:px-3.5 py-1 sm:py-1.5 rounded-lg bg-gradient-to-r from-blue-600 to-brand-600 text-white text-[11px] sm:text-xs font-bold shadow-md shimmer-sweep"
              >
                RFQ
              </button>
              ...
              <button
                ...
                className="p-1.5 sm:p-2 rounded-xl bg-brand-800 text-slate-200 hover:text-white hover:bg-brand-700 focus:outline-none border border-brand-700/60 transition-colors"
                aria-label="Toggle navigation menu"
                aria-expanded={mobileMenuOpen}
              >
                {mobileMenuOpen ? (
                  <FontAwesomeIcon icon={faXmark} className="text-brand-300 text-base sm:text-lg" aria-hidden="true" />
                ) : (
                  <FontAwesomeIcon icon={faBars} className="text-brand-300 text-base sm:text-lg" aria-hidden="true" />
                )}
              </button>
            </div>
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden overflow-hidden bg-brand-900/98 border-b border-white/10 backdrop-blur-2xl"
            >
              <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 space-y-1.5 max-h-[80vh] overflow-y-auto">
                  ...
                  return (
                    <motion.a
                      ...
                      className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all ${
                        isActive
                          ? 'bg-brand-700/60 text-brand-200 border border-brand-500/40'
                          : 'text-slate-200 hover:text-brand-300 hover:bg-brand-800/70'
                      }`}
                    >
                      <span>{link.name}</span>
                ...
                <div className="pt-3 mt-2 border-t border-brand-700/60 space-y-2">
                  <button
                    onClick={() => {
                      setMobileMenuOpen(false);
                      onOpenQuoteModal();
                    }}
                    className="w-full py-2.5 sm:py-3 rounded-xl bg-gradient-to-r from-blue-600 via-brand-500 to-brand-600 text-white font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg shimmer-sweep"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-2 sm:py-2.5 rounded-xl bg-emerald-950/50 hover:bg-emerald-900/60 border border-emerald-500/30 text-emerald-300 text-xs font-semibold flex items-center justify-center gap-2 transition-all"
                  >
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-400 pt-1 px-1">
                    <span className="flex items-center gap-1 truncate">
                      <FontAwesomeIcon icon={faEnvelope} className="text-brand-400 text-xs" />
                      {COMPANY.contact.email}
                    </span>
                    <span className="text-emerald-400 font-mono shrink-0 ml-2">24/7 Response</span>
                  </div>
                </div>
              </div>
            </motion.div>
